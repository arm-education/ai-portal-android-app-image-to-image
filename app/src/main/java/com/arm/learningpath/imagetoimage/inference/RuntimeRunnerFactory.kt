...
object RuntimeRunnerFactory {
    fun create(context: Context, config: ModelConfig): RuntimeRunner {
        return when (config.adapterId) {
            "mobile-sam-executorch" -> MobileSamExecuTorchAdapter()
            "depth-anything-v2-executorch" -> DepthAnythingExecuTorchAdapter()
            else -> when (config.runtime) {
